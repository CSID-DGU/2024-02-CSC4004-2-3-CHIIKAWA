...
const ChatRoom = () => {
  ...
  return (
    <div className="chat-room-container">
      <header>
        <IconButton color="primary" className="icon-button" onClick={() => setIsLeftSidebarOpen(!isLeftSidebarOpen)}>
          <ChatIcon />
        </IconButton>
        <div className="chat-room-title">{roomName}</div>
        <IconButton color="primary" className="icon-button" onClick={() => setIsRightSidebarOpen(!isRightSidebarOpen)}>
          <PeopleAltIcon />
        </IconButton>
      </header>
          ...
          {messages.map((msg) => (
            <div className="chat-message-container" key={msg.id}>
              <div className="avatar-message">
                <AccountCircleOutlinedIcon sx={{ fontSize: 40, color: "rgb(102, 102, 102)" }} />
              </div>
